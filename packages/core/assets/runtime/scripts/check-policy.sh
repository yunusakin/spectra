...
Checks:
- Approval gate: no non-README app code unless intake-state Approval Status is `implementation-approved` or `release-approved`
...
if [[ "${approval_status}" != "not approved" && -n "${approval_status}" && ${open_count} -gt 0 ]]; then
  add_error "Approval Status is set while there are open technical questions in ${state_file}."
fi
