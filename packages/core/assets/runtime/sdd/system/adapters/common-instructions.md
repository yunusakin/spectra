...
- Use route-first context for normal development: run `./.spectra/bin/spectra route --task "<task description>"` before broad exploration and load only selected module/domain context.
...
- Require verified evidence before active business knowledge; use `./.spectra/bin/spectra knowledge add --status active --verified` only for authoritative rules.
