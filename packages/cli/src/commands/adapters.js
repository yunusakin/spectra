...
import { checkAgentsHealth, findForeignAdapterFiles, normalizeAgents } from "../lib/agent-health.js";
...
function adaptersCommand(argv) {
  ...
  if (options["--help"]) {
    title("Usage: spectra adapters --agents <csv> [--cwd <path>] [--target <path>] [--force]");
    return 0;
  }
  ...
  if (!options["--force"]) {
    const foreign = findForeignAdapterFiles(
      path.resolve(options["--target"] ?? cwd),
      normalizeAgents(options["--agents"])
    );
    if (foreign.length > 0) {
      throw new Error(
        `Refusing to overwrite existing file(s) not generated by Spectra: ${foreign.join(", ")}. Move them aside or pass --force.`
      );
    }
  }
