...
function knowledgeCommand(argv) {
  ...
  const { options } = parseOptions(rest, {
    booleanFlags: ["--help"],
    stringFlags: ["--cwd", "--domain", "--id", "--statement", "--status", "--title"]
  });
  ...
  if (action === "add") {
    ...
    const result = addBusinessRule({ cwd: options["--cwd"] ?? process.cwd(), domain: options["--domain"], title: options["--title"], statement: options["--statement"], status: options["--status"] ?? "unresolved" });
